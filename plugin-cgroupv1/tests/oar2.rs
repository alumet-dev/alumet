...
mod tests {
    ...
    #[test]
    fn test_runtime() -> Result<(), Box<dyn std::error::Error>> {
        let mut plugins = PluginSet::new();
        let temp_dir = tempdir().unwrap();
        ...
        let runtime_expectations = RuntimeExpectations::new()
            ...
            .test_source(SourceName::from_str("oar2-plugin", "somesubmitter_5678"), || (), {
                ...
                move |m| {
                    ...
                    let job_id_attribute = attributes[0];
                    ...
                    if let Err(err) = create_oar_mocks(&path, mocks) {
                        println!("error while creating oar mock: {err:?}");
                        return;
                    };
