...
impl Transform for EnergyAttributionTransform {
    fn apply(
        ...
        while self.buffer_rapl.len() >= 2 && self.buffer_pod.len() >= 2 {
            let rapl_mini_id = self.buffer_rapl.keys().reduce(|x, y| if x < y { x } else { y }).unwrap().clone();
            ...
            let utime_sum = self.buffer_pod
                .get(&rapl_mini_id).unwrap()
                .iter().map(|x| x.clone().value)
                .reduce(|x, y| {
                    match (x, y) {
