...
mod tests {
    ...
    // Test `get` function with valid device
    #[ignore = "NO GPU"]
    #[test]
    fn test_get_with_valid_device() {
        let devices = NvmlDevices::detect(false).expect("Device recognizing");
        if let Some(device) = devices.get(0) {
            assert!(!device.name().expect("Device name").is_empty());
        } else {
            panic!("Expected valid device at index 0");
        }
    }

    // Test `get` function without valid device
    #[ignore = "NO GPU"]
    #[test]
    fn test_get_with_invalid_device() {
        let devices = NvmlDevices::detect(false).expect("Device recognizing");
        assert!(devices.get(devices.devices.len()).is_none());
    }
