    ...
    match read_dir(dir_path) {
        Ok(dir) => {
            for entry in dir {
                ...
                if entry.path().canonicalize()?.is_dir() {
...
fn detect_hierarchy_modern<P: AsRef<Path>>(sys_ina: P) -> anyhow::Result<Vec<InaSensor>> {
    ...
    fn sensor_channels_dir(sensor_path: &Path) -> anyhow::Result<PathBuf> {
        ...
        for child in
            read_dir(&hwmon).with_context(|| format!("failed to list content of directory {}", hwmon.display()))?
        {
            let child = child?;
            let path = child.path();
...
mod tests {
    ...
    fn test_detect_hierarchy_modern() {
        let root = temp_dir().join("test-alumet-plugin-nvidia/ina-modern");
        if root.exists() {
    ...
    fn test_detect_hierarchy_old_v4() {
        ...
        let root = temp_dir().join("test-alumet-plugin-nvidia/ina-old");
        if root.exists() {
            remove_dir_all(&root).unwrap();
        }
