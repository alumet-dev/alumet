...
pub struct AmdGpuProbe {
    ...
    /// Metric type based on GPU fan speed data.
    pub fan_speed: TypedMetricId<u64>,
    ...
    fn poll(&mut self, measurement: &mut MeasurementAccumulator, timestamp: Timestamp) -> Result<(), PollError> {
        for device in gather_amd_gpu_device_measurements()? {
            ...
            if let Some(id) = device.metrics.bus_id.clone() {
                ...
                // GPU fan speed metric pushed
                if device.available.gpu_fan_speed {
                    if let Some(value) = device.metrics.fan_speed {
                        measurement.push(MeasurementPoint::new(
                            timestamp,
                            self.fan_speed,
                            resource.clone(),
                ...
                if device.available.gpu_pci_data_sent {
                    if let Some(value) = device.metrics.pci_data_sent {
                        measurement.push(MeasurementPoint::new(
                            timestamp,
                            self.pci_data_sent,
                            resource.clone(),
                ...
                if device.available.gpu_pci_data_received {
                    if let Some(value) = device.metrics.pci_data_received {
                        measurement.push(MeasurementPoint::new(
                            timestamp,
                ...
                for (data, clock_type) in &device.metrics.clock_frequencies {
                    if device.available.gpu_clock_frequencies {
