...
use amdsmi::{
    ...
    amdsmi_get_gpu_compute_process_info_by_pid, amdsmi_get_gpu_device_bdf, amdsmi_get_gpu_memory_usage,
    amdsmi_get_gpu_pci_throughput, amdsmi_get_power_info, amdsmi_get_processor_handles, amdsmi_get_socket_handles,
    amdsmi_get_temp_metric, amdsmi_shut_down, AmdsmiClkTypeT, AmdsmiMemoryTypeT, AmdsmiTemperatureMetricT,
...
pub struct AmdGpuMetrics {
    ...
    pub clock_frequencies: Vec<(Features<u64>, String)>,
    ...
    pub memory_usages: Vec<(Features<u64>, String)>,
    ...
    pub temperatures: Vec<(Features<u64>, String)>,
}
...
pub fn gather_amd_gpu_process_measurements() -> Result<Vec<AmdGpuProcessMetrics>, PollError> {
    ...
    // Get the number of running GPU compute processes
    ...
    if let Some((procs, num_items)) = data.value {
        for proc in procs {
            ...
            if let Some(data) = item.value {
                let metric = AmdGpuProcessMetrics {
                    pid: Some(pid),
                    counter: Some(Features::new(Some(num_items as u64), true)),
                    compute_unit_usage: Some(Features::new(Some(data.cu_occupancy as u64), true)),
                    vram_usage: Some(Features::new(Some(data.vram_usage / 1_000_000), true)),
                };
                metrics.push(metric);
            }
        }
