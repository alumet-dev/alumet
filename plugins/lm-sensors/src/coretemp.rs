...
fn resource_from_string(name: &str, coretemp_id: u32) -> anyhow::Result<Resource> {
    ...
    match v[0] {
        "Package" => {
            if let Ok(id) = v
                .get(2)
                .context("Failed to parse the coretemp sensor name {name}, expected a Package id value")?
                .parse::<u32>()
            {
        ...
        "Core" => {
            if let Ok(id) = v
                .get(1)
                .context("Failed to parse the coretemp sensor name {name}, expected a Core id value")?
                .parse::<u32>()
            {
...
pub fn get_coretemp_sensors_list<'a>(
    lmsensors: &'a LMSensors,
    package_only: bool,
) -> anyhow::Result<Vec<CoretempSensor<'a>>> {
    ...
    for chip in lmsensors.chip_iter(None).filter(|x| {
        ...
        x.name().unwrap_or_default().starts_with("coretemp-isa-")
    }) {
            ...
            .parse()
        {
            ...
            Err(_) => {
                return Err(anyhow!(
                    "Unexpected coretemp name ({}) should be of the form 'coretemp-isa-XXXX' with 'XXXX' being the package id",
                    chip.name().unwrap()
                ));
            }
        };
        ...
        let tmp_sensors_list: anyhow::Result<Vec<CoretempSensor<'a>>> = chip
            ...
            .filter(|x| {
                !package_only
                    || x.label().map_or_else(
                        |_| true, // Silently pass the error to be treated in CoretempSensor constructor
