...
- If you are on a cluster and run alumet in OAR or SLURM job, you must have root permissions to run properly all metrics, and need to be included in groups `render` and `video` (especially for processes information, power and energy consumptions).
...
int main() {
    ...
    // Allocation and initialization to an array on the host
    ...
    // Copy data from the host to GPU
    hipMemcpy(d_A, h_A, size, hipMemcpyHostToDevice);

    // Blocs and grid Configuration for kernel
    ...
    // Get modified data from GPU to host
