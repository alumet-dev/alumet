...
use crate::{
    ...
    interface::{MockAmdSmiTrait, MockProcessorHandleTrait, MockSocketHandleTrait, set_amd_smi_factory},
    tests::mocks::{
...
fn test_start_success() {
    set_amd_smi_factory(|| {
        ...
        mock_init.expect_lib_stop().returning(|| Ok(()));

        Ok(Box::new(mock_init))
    });
