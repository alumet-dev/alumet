...
use amdsmi::{
    ...
    amdsmi_get_temp_metric, amdsmi_is_gpu_power_management_enabled, amdsmi_get_gpu_process_list,
    AmdsmiMemoryTypeT, AmdsmiTemperatureMetricT, AmdsmiTemperatureTypeT, AmdsmiStatusT,
};
use std::ffi::c_void;
...
impl OptionalFeatures {
    ...
    pub fn detect_on(device: *mut c_void) -> Result<Self, AmdsmiStatusT> {
        ...
        for &(sensor, _) in &SENSOR_TYPE {
            let supported = is_supported(amdsmi_get_temp_metric(device, sensor, AmdsmiTemperatureMetricT::AmdsmiTempCurrent))?;
...
mod tests {
    ...
    fn test_fmt_sucess() {
        ...
        features.gpu_memory_usages.insert(AmdsmiMemoryTypeT::AmdsmiMemTypeVram, true);
        features.gpu_temperatures.insert(AmdsmiTemperatureTypeT::AmdsmiTemperatureTypeEdge, true);
    ...
    // Test `is_supported` function with success
    #[test]
    fn test_is_supported_ok() {
        let ret: Result<i32, AmdsmiStatusT> = Ok(42);
    ...
    // Test `has_any` function with all features available
    #[test]
    fn test_has_any_all_true() {
        let mut features = OptionalFeatures::default();
        features.gpu_energy_consumption = true;
        features.gpu_engine_usage = true;
        features.gpu_power_consumption = true;
        features.gpu_state_management = true;
        features.gpu_process_info = true;
        features.gpu_memory_usages.insert(AmdsmiMemoryTypeT::AmdsmiMemTypeGtt, true);
        features.gpu_temperatures.insert(AmdsmiTemperatureTypeT::AmdsmiTemperatureTypeEdge, true);
        assert!(features.has_any());
    }

    // Test `with_detected_features` function with null GPU device handler
    #[test]
    fn test_with_detected_features_with_null_device() {
        let handle = ptr::null_mut();
        let res = OptionalFeatures::with_detected_features(handle);
        match res {
            Ok((device, features)) => {
                assert_eq!(device, handle);
                assert!(!features.has_any());
            },
            Err(e) => {
                assert_eq!(e, AmdsmiStatusT::AmdsmiStatusNotInit);
            }
        }
    }
