    ...
    T: Copy + Add<Output = T> + Sub<Output = T> + PartialOrd + Default,
{
    pub fn compute(&self) -> Option<T> {
        if (self.cpu_value.is_none() || self.grace_value.is_none() || self.sysio_value.is_none()) {
            log::warn!("Can't compute DRAM value");
            return None;
        }
        let grace = self.grace_value.unwrap_or(T::default());
        let cpu = self.cpu_value.unwrap_or(T::default());
        let sysio = self.sysio_value.unwrap_or(T::default());
