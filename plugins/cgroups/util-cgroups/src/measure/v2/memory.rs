...
use crate::measure::{
    parse::{SelectiveStatFile, StatFileBuilder, U64File, U64MaxResult, ValueFile},
    v2::{
...
impl MemoryMaxCollector {
    ...
    pub fn measure(&mut self, io_buf: &mut Vec<u8>) -> io::Result<f64> {
        // SAFETY: the content is generated by the kernel and is always valid ASCII (hence valid UTF-8)
        match unsafe { self.file.read(io_buf) }? {
            U64MaxResult::U64(value) => Ok(value as f64),
            U64MaxResult::Max => {
                let value = self.file.maximum_value.unwrap_or(-1.0);
...
mod tests {
    ...
    fn collect_memory_swap_max() -> anyhow::Result<()> {
        ...
        let res = collector.measure(io_buf.as_mut())?;
        assert_eq!(res, 343597385987368.0);
