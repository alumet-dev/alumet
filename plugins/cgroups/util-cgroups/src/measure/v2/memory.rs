...
use crate::measure::{
    parse::{SelectiveStatFile, StatFileBuilder, U64File, U64MaxResult, ValueFile, total_memory, total_swap},
    v2::{
...
impl MemoryMaxCollector {
    ...
    pub fn measure(&mut self, io_buf: &mut Vec<u8>) -> io::Result<u64> {
        // SAFETY: the content is generated by the kernel and is always valid ASCII (hence valid UTF-8)
        match unsafe { self.file.read(io_buf) }? {
            U64MaxResult::U64(value) => Ok(value),
            U64MaxResult::Max => {
                let value = self.file.maximum_value.unwrap_or(total_memory()?);
                self.file.maximum_value = Some(value);
                Ok(value)
            }
        }
    }
...
impl MemorySwapMaxCollector {
    ...
    pub fn measure(&mut self, io_buf: &mut Vec<u8>) -> io::Result<u64> {
        ...
        match unsafe { self.file.read(io_buf) }? {
            U64MaxResult::U64(value) => Ok(value),
            U64MaxResult::Max => {
                let value = self.file.maximum_value.unwrap_or(total_swap()?);
                self.file.maximum_value = Some(value);
                Ok(value)
            }
        }
...
mod tests {
    ...
    use crate::measure::v2::{
        memory::{
            MemoryCurrentCollector, MemoryMaxCollector, MemoryStatCollector, MemoryStatCollectorSettings,
            MemorySwapCurrentCollector, MemorySwapMaxCollector,
        },
    ...
    #[test]
    fn collect_memory_swap_current() -> anyhow::Result<()> {
        let tmp = tempfile::NamedTempFile::new()?;

        let mut io_buf = Vec::new();
        let mut collector = MemorySwapCurrentCollector::new(tmp.path())?;
        ...
        std::fs::write(tmp.path(), "858555755622")?;
        let res = collector.measure(io_buf.as_mut())?;
        assert_eq!(res, 858555755622);

        Ok(())
    }

    #[test]
    fn bad_memory_swap_current() -> anyhow::Result<()> {
        let tmp = tempfile::NamedTempFile::new()?;
        ...
        std::fs::write(tmp.path(), "not_a_number")?;
        let res = collector.measure(io_buf.as_mut()).expect_err("should fail");
        assert_eq!(res.kind(), ErrorKind::InvalidData);
    ...
    #[test]
    fn collect_memory_swap_max() -> anyhow::Result<()> {
        let tmp = tempfile::NamedTempFile::new()?;

        let mut io_buf = Vec::new();
        let mut collector = MemorySwapMaxCollector::new(tmp.path())?;

        std::fs::write(tmp.path(), "8589934592")?;
        let res = collector.measure(io_buf.as_mut())?;
        assert_eq!(res, 8589934592);
        ...
        // Test with very large values
        std::fs::write(tmp.path(), "343597385987368")?;
        let res = collector.measure(io_buf.as_mut())?;
        assert_eq!(res, 343597385987368);

        Ok(())
    }

    #[test]
    fn bad_memory_swap_max() -> anyhow::Result<()> {
        let tmp = tempfile::NamedTempFile::new()?;
