...
fn csv_output() {
    ...
    let result_file: String = file_path.clone().into_os_string().into_string().unwrap();
    ...
    let runtime_expectations = RuntimeExpectations::new()
        ...
        .test_output(
            ...
            },
            {
                move || {
                    assert_eq!(
                        fs::read_to_string(result_file.as_str()).unwrap(),
        ...
        .test_output(
            ...
            },
            {
                move || {
                    assert_eq!(
                        fs::read_to_string(result_file_2.as_str()).unwrap(),
...
fn write_output_config_unit_unique_name() {
    ...
    let runtime_expectations = RuntimeExpectations::new()
        ...
        .test_output(
            ...
            {
                move || {
                    assert_eq!(fs::read_to_string(result_file.as_str()).unwrap(), expected_string);
                }
            },
        );
    let agent = agent::Builder::new(plugins)
