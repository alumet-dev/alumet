...
use crate::plugins::oar::job_tracker::JobTracker;
...
impl Transform for JobInfoAttacher {
    fn apply(&mut self, measurements: &mut MeasurementBuffer, _ctx: &TransformContext) -> Result<(), TransformError> {
        // lazily initialized
        let current_job_list = LazyCell::new(|| {
            self.tracker
                .known_jobs_sorted()
                .into_iter()
                .map(|id| id.to_string())
        ...
        for m in measurements.iter_mut() {
            if !m.attributes_keys().any(|k| k == "job_id") {
                ...
                m.add_attr("jobs_ids", AttributeValue::String(jobs_attr));
                // TODO check the format of the attribute with bruno
            }
        }
        Ok(())
    }
