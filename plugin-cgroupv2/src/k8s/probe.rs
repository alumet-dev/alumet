    ...
    fn poll(&mut self, measurements: &mut MeasurementAccumulator, timestamp: Timestamp) -> Result<(), PollError> {
        ...
        if let Some(value_tot) = diff_tot {
            let p_tot: MeasurementPoint = self.create_measurement_point(
                timestamp, self.time_used_tot, consumer_cpu.clone(), value_tot, &metrics);
        ...
        if let Some(value_usr) = diff_usr {
            let p_usr: MeasurementPoint = self.create_measurement_point(
                timestamp, self.time_used_user_mode, consumer_cpu.clone(), value_usr, &metrics);
        ...
        if let Some(value_sys) = diff_sys {
            let p_sys: MeasurementPoint = self.create_measurement_point(
                timestamp, self.time_used_system_mode, consumer_cpu.clone(), value_sys, &metrics);
            measurements.push(p_sys);
        }

        // Push anonyme used memory measure corresponding to running process and various allocated memory
        let diff_mem_anon_value = metrics.anon_used_mem;
        let m_anon: MeasurementPoint = self.create_measurement_point(
            timestamp, self.anon_used_mem, consumer_memory.clone(), diff_mem_anon_value / 1000, &metrics);
        ...
        let m_file: MeasurementPoint = self.create_measurement_point(
            timestamp, self.file_mem, consumer_memory.clone(), diff_mem_file_value / 1000, &metrics);
        ...
        let m_shared: MeasurementPoint = self.create_measurement_point(
            timestamp, self.shared_mem, consumer_memory.clone(), diff_mem_shared_value / 1000, &metrics);
        ...
        let m_file_mapped: MeasurementPoint = self.create_measurement_point(
            timestamp, self.file_mapped_mem, consumer_memory.clone(), diff_mem_file_mapped_value / 1000, &metrics);
        measurements.push(m_file_mapped);

        // Push total memory used by cgroup measure
        let diff_mem_total_value = (diff_mem_anon_value + diff_mem_file_value + diff_mem_shared_value + diff_mem_file_mapped_value) / 1000;
        let m_total: MeasurementPoint = self.create_measurement_point(
            timestamp, self.total_mem, consumer_memory.clone(), diff_mem_total_value, &metrics);
