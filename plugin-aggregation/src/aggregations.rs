...
pub (crate) fn mean(sub_vec: Vec<MeasurementPoint>) -> WrappedMeasurementValue {
    ...
    match result {
        WrappedMeasurementValue::F64(fx) => {
            WrappedMeasurementValue::F64(fx / sub_vec.len() as f64)
        },
        WrappedMeasurementValue::U64(ux) => {
            WrappedMeasurementValue::U64(ux / sub_vec.len() as u64)
        },
    }
}
